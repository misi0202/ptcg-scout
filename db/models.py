...
def insert_card(conn: sqlite3.Connection, name: str, set_name: str,
                ...
                artist: str = "", rarity: str = "") -> int:
    conn.execute(
        """INSERT OR IGNORE INTO cards (name, set_name, card_number, pokemon_name, artist, rarity)
           VALUES (?, ?, ?, ?, ?, ?)""",
        (name, set_name, card_number, pokemon_name, artist, rarity),
    )
    row = conn.execute(
